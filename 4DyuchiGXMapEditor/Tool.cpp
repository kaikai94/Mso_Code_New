...
BOOL CTool::Initialize(CWnd* pWnd)
{
	...
	HMODULE Executive = 0;
	Executive = LoadLibrary("Executive.dll");
	if(!Executive)
		__asm int 3
	CREATE_INSTANCE_FUNC        pFunc;
	pFunc = (CREATE_INSTANCE_FUNC)GetProcAddress(Executive,"DllCreateInstance");
	hr = pFunc((void**)&m_pExecutive);

	if (hr != S_OK)
		__asm int 3

		m_pExecutive->InitializeFileStorageWithoutRegistry("FileStorage.dll",35000,
		10000,
		MAX_PATH,
	...
	m_pExecutive->InitializeWithoutRegistry("Geometry.dll","Renderer.dll",
		pWnd->m_hWnd, NULL, 10000, 300,0,1, 0,NULL);
...
void CTool::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	bDirty = FALSE;
	DWORD viewType = 0;
	if(m_Mouse.MoveMouse( nFlags, point.x, point.y,viewType))
	{
		Render();
		goto lb_return;
	}

...
void CFortWarManager::AddSymbolNpc( float x, float z)
{
	...
	binfo.ObjectState = 0;
	SafeStrCpy( binfo.ObjectName, "¿ÌÓ¡Ê¯", MAX_NAME_LENGTH+1 );
	...
	case MP_FORTWAR_ING:
		{
			...
			m_WarStart = TRUE;//[ÐÞ¸´ÒªÈûºó½øÈëÎÞ·¨¹¥»÷][by:Ê®ÀïÆÂ½£Éñ][QQ:112582793][2019/1/10]
		}
		break;
	...
	case MP_FORTWAR_WAREHOUSE_ITEM_MOVE_NACK:
		{
			...
			{
			case eFortWarWareError_None:	CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1753 ) );	break;	// CHATMGR->AddMsg( CTC_SYSMSG, "[¿ä»õÃ¢°í] »ç¿ëÇÒ ¼ö ¾ø½À´Ï´Ù." );	break;
			case eFortWarWareError_NotMap:	CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1754 ) );	break;	// CHATMGR->AddMsg( CTC_SYSMSG, "[¿ä»õÃ¢°í] »ç¿ëÇÒ ¼ö ÀÖ´Â ¸ÊÀÌ ¾Æ´Õ´Ï´Ù." );	break;
			case eFortWarWareError_NotTime:	CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1755 ) );	break;	// CHATMGR->AddMsg( CTC_SYSMSG, "[¿ä»õÃ¢°í] »ç¿ëÇÒ ¼ö ÀÖ´Â ½Ã°£ÀÌ ¾Æ´Õ´Ï´Ù." );	break;
			case eFortWarWareError_NotRight:	CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1756 ) );	break;	// CHATMGR->AddMsg( CTC_SYSMSG, "[¿ä»õÃ¢°í] »ç¿ëÇÒ ±ÇÇÑÀÌ ¾ø½À´Ï´Ù." );	break;
			case eFortWarWareError_NotUse:	CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1753 ) );	break;	// CHATMGR->AddMsg( CTC_SYSMSG, "[¿ä»õÃ¢°í] »ç¿ëÇÒ ¼ö ¾ø½À´Ï´Ù." );	break;
			}
...
//			CHATMGR->AddMsg( CTC_SYSMSG, "°ø¼ºÃ¢°í·Î ¾ÆÀÌÅÛÀÌ ÀÌµ¿ÇÏ¿´½À´Ï´Ù." );
		}
		break;
	...
	case MP_FORTWAR_WAREHOUSE_ITEM_MOVE_TO_SIEGEHOUSE:
		{
			...
			if( pFortWarWareHouseDlg->IsActive() )
			{
				if( pFortWarWareHouseDlg->GetDialogKind() == eFortWareHouseDlg )		// ¿ä»õÃ¢°í¸¦ º»´Ù¸é...
				{
					CItem* pFromItem = NULL;
					pFortWarWareHouseDlg->DeleteItem( pmsg->wFromPos, &pFromItem );
					ITEMMGR->ItemDelete( pFromItem );
...
//				CHATMGR->AddMsg( CTC_SYSMSG, "¿ä»õÃ¢°í·Î %s ¾ÆÀÌÅÛÀÌ µå¶øµÇ¾ú½À´Ï´Ù.", pInfo->ItemName );
			}
		}
		break;
	}

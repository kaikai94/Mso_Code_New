...
{
	// ¾ÆÀÌÅÛÀ» ¹Þ´Â ÁßÀÌ¸é ±â´Ù¸². 
	// ¾ÆÀÌÅÛ ´Ù ¹ÞÀ¸¸é ¶ô Ç®±â 
	// ´Ù¸¥ ¹öÆ°ÀÌ ¼±ÅÃµÇ¸é ÇöÀç ÅÇ MSG_BYTE·Î MP_ITEM_GUILD_WAREHOUSE_LEAVE º¸³¿ 
	// ¹öÆ° ´­·ÁÁö¸é ¾ÆÀÌÅÛ º¸³»´Þ¶ó°í ¿äÃ». ¹Þ±â Àü¿¡ ´Ù¸¥ ÅÇ ¸ø ´©¸£°Ô ¶ô 
...
void CGuildWarehouseDialog::OnActionEvent(LONG lId, void * p, DWORD we)
{
	...
	if (we == WE_RBTNCLICK)
	{
		//[ÓÒ¼ü²Ö´¢][BY:Ê®ÀïÆÂ½£Éñ][QQ:112582793][2019-8-24][19:09:06]
		POSTYPE pos = 0;
...
BOOL CGuildWarehouseDialog::FakeMoveItem( LONG mouseX, LONG mouseY, CItem * pFromItem )
{
	...
	{
		CHATMGR->AddMsg(CTC_SYSMSG, "Å¸ÀÌÅº Áõ¼­´Â ¹®ÆÄÃ¢°í¿¡ ³ÖÀ»¼ö ¾ø½À´Ï´Ù.");
...
BOOL CGuildWarehouseDialog::FakeGeneralItemMove( POSTYPE ToPos, CItem * pFromItem, CItem * pToItem )
{
	...
	{//[ÅÝµãÎïÆ·½ûÖ¹·ÅÈëÃÅÅÉ²Ö¿â][By:Ê®ÀïÆÂ½£Éñ][QQ:112582793][2017/12/16]
...
BOOL CGuildWarehouseDialog::AddItem(ITEMBASE* pItemInfo)
{
	...
	{
		ASSERTMSG(0,"±æµåÃ¢°í-¾ÆÀÌÅÛ µðºñÀÎµ¦½º°¡ 0. Invalid GuildWarehouse Index.");
		return FALSE;
	}

...
BOOL CFilteringTable::FilterChat( char* pChat )
{
	...
	strupr( m_strBuf );
	if(IsSendItemInfoMsg(pChat))return FALSE;
...
BOOL CFilteringTable::IsSendItemInfoMsg(char * pStr)
{
	std::string strBuff = pStr;
	WORD count = 0;
	if(strBuff.length()>0)
	{
		int FindStartPos=0;
		while(FindStartPos>=0 && FindStartPos< strBuff.length())
		{
			int FindPos= strBuff.find('&',FindStartPos);

			if(FindPos>=0)
			{
				std::basic_string <char> ImgStr = strBuff.substr(FindPos+1,2);

				std::basic_string <char> FlagStr= strBuff.substr(0,FindPos);

...
void cTabDialog::RenderTabComponent()
{
	...
	for(int i = 0 ; i < m_bTabNum ; i++)
	{
		if (m_bSelTabNum == i&&m_ppWindowTabSheet[i]!=NULL)
		{
			m_ppWindowTabSheet[i]->Render();
		}
		if (m_ppPushupTabBtn[i]!=NULL)
...
void cTabDialog::SetPushButtonHide(BYTE idx,BOOL val)
{
	ASSERT(idx < m_bTabNum);
	m_ppPushupTabBtn[idx]->SetActive(val);
}

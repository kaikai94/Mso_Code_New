...
static const DWORD gColor[] = { RGBA_MAKE(255, 255, 255, 255),
								RGBA_MAKE(252, 255, 110, 255), 
								RGBA_MAKE(252, 0, 0, 255), 
								RGBA_MAKE(0, 255, 0, 255)};
static const DWORD changeTime = 100;
cTextArea::cTextArea()
{
	m_nTopLineIdx	= 0;
	m_nLineNum		= 0;
...
//#ifdef _BIG5SYSTEM_					
//					if (strlen(strBuf)>=2)
//						CFONT_OBJ->RenderFont( m_wFontIdx, CODEMGR->ChangeCharCode(strBuf), nStrLen, &rcText, 
//						RGBA_MERGE( m_dwTextColor, m_alpha * m_dwOptionAlpha / 100 ) );					
//					else
//#endif
					if (m_bIsChangeColor)
					{
						if (GetTickCount() >= m_ColorTime)
						{
							m_dwTextColor = gColor[m_ColorIndex];
							m_ColorTime = GetTickCount() + changeTime;

							if (m_ChangeMode == 0)
								m_ColorIndex++;
							else
								m_ColorIndex--;
							if (m_ColorIndex >= 4 && m_ChangeMode == 0)
							{
								m_ChangeMode = 1;
								m_ColorIndex = 3;
							}
							else if (m_ColorIndex <= 0 && m_ChangeMode == 1)
							{
								m_ChangeMode = 0;
							}
						}
...
void cTextArea::GetScriptText( char* outText )
{
	...
	if(strlen(outText)>=2)
	{
		CODEMGR->ChangeCharCode(outText);
	}
#endif
}

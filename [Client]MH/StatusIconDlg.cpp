...
#include "cMsgBox.h"		//右键删除buff引用	by:十里坡剑神	QQ:112582793
#include "WindowIDEnum.h"	//右键删除buff引用	by:十里坡剑神	QQ:112582793
//////////////////////////////////////////////////////////////////////
...
CStatusIconDlg::CStatusIconDlg()
{
	...
	m_nQuestIconCount = 0;
	m_Show = TRUE;	//[buff图标隐藏标志][By:十里坡剑神][QQ:112582793][2017/11/22]
}
...
void CStatusIconDlg::Render()
{
	if(m_pObject == NULL)
		return;
	//[buff图标隐藏标志][By:十里坡剑神][QQ:112582793][2017/11/22]
	if(!m_Show)
		return;
#ifdef _GMTOOL_
	...
	for(int n=1;n<eStatusIcon_Max;++n)
	{
			...
			if(0 < dx && dx < 35 && 0 < dy && dy < 35)
			{
				if(m_IconInfo[n].ItemIndex)
				{
					...
					if(pInfo)
					{
						...
						{//[成长符倍数显示][BY:十里坡剑神][QQ:112582793][2019-8-9][14:17:58]
				...
				else
				{
					...
					else
					{
							...
							|| n == eStatusIcon_GuildPlustime_Exp || n == eStatusIcon_GuildPlustime_DamageUp )
						{
							...
							m_toolTip.AddLine(CHATMGR->GetChatMsg(766), TTTC_DEFAULT);
							sprintf(buf, CHATMGR->GetChatMsg(767),	//符咒15年
								EndTime.GetYear()/*+15*/, EndTime.GetMonth(), EndTime.GetDay(), EndTime.GetHour(), EndTime.GetMinute());
							m_toolTip.AddLine(buf, TTTC_QUESTITEM);
						}
						else if( n == 148 )
						{
							...
							{//[组队物品掉落加成][By:十里坡剑神][QQ:112582793][2018/4/22]
							...
							{//[组队内力恢复加成][By:十里坡剑神][QQ:112582793][2018/4/22]
							...
							{//[组队无双恢复速度加成][By:十里坡剑神][QQ:112582793][2018/4/22]
							...
							{//[组队防御力加成][By:十里坡剑神][QQ:112582793][2018/4/22]
							...
							{//[组队一击概率加成][By:十里坡剑神][QQ:112582793][2018/4/22]
		...
		if(CanDelBuf)
		{
			if(g_UserInput.GetMouse()->RButtonDown())//右键删除buff判断	by:十里坡剑神	QQ:112582793
			{
				int dx = int(mx - curpos.x);
				int dy = int(my - curpos.y);	
				if(0 < dx && dx < 35 && 0 < dy && dy < 35)

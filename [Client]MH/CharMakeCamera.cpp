...
BOOL CCharMakeCamera::MoveCamera(int iLastX, int iLastY, int iCurrentX, int iCurrentY)
{
	...
	v3CameraAngle.x	*=	CHARMAKEMGR->GetMakeCfg().fCameraAngleX; // 현재 xrotate가 반대로 나온다. 언제 고쳐질지 몰라. 
...
{
	if (CHARMAKEMGR->GetMakeCfg().fZoom[1]<= m_fZoom)
	{
		return FALSE;
	}
	...
	const float c_fMoveDistance = CHARMAKEMGR->GetMakeCfg().fMonveDistance;
...
{
	if (CHARMAKEMGR->GetMakeCfg().fZoom[0] >= m_fZoom)
	{
		return FALSE;
	}
...
BOOL CCharMakeCamera::RotateCamera(GXOBJECT_HANDLE gxh, int iLastX, int iLastY, int iCurrentX, int iCurrentY)
{
	...
	const float c_fRotateAngle = CHARMAKEMGR->GetMakeCfg().fRotateAngle;
	...
	if (0 > iVertMove)
	{
		if (CHARMAKEMGR->GetMakeCfg().fRotPitch[1] <= m_fPitchRot)
		{
			return FALSE;
		}
	}
	else if (0 < iVertMove)
	{
		...
		if (CHARMAKEMGR->GetMakeCfg().fRotPitch[0] >= m_fPitchRot)
		{
			return FALSE;
		}
	}
	...
	//SW 값 보정.
	if( v3CameraAngle.x < PI * -CHARMAKEMGR->GetMakeCfg().fPI[0] )
		v3CameraAngle.x += CHARMAKEMGR->GetMakeCfg().fPI[1] * PI;
	else if( v3CameraAngle.x > PI * CHARMAKEMGR->GetMakeCfg().fPI[0])
		v3CameraAngle.x -= CHARMAKEMGR->GetMakeCfg().fPI[1] * PI;
...
{	
	...
	m_pExecutive->GetGeometry()->ResetCamera(&CHARMAKEMGR->GetMakeCfg().vCameraEye, 
											 DEFAULT_NEAR/10.0f,
											 DEFAULT_FAR/10.0f,
											 DEFAULT_FOV,
											 0);

...
void CUserInfoManager::LoadInterfaceInfo( char* strFilePath )
{
	...
	if( bLowResolution != GAMERESRCMNGR->GetPrintScreenIndex() )
	{
		CloseHandle( hFile );
		return;
	}
...
//----
	...
	int bLowResolution = GAMERESRCMNGR->GetPrintScreenIndex();
...
BOOL CUserInfoManager::IsValidInterfacePos( RECT* prcCaption, LONG lX, LONG lY )
{
	...
	if( lX + prcCaption->right > GAMERESRCMNGR->m_GameDesc.dispInfo.dwWidth ) return FALSE;  
	if( lY + prcCaption->top < 0 )		return FALSE;
	if( lY + prcCaption->bottom > GAMERESRCMNGR->m_GameDesc.dispInfo.dwHeight ) return FALSE;

...
BOOL CDamageEffectUnit::On(EFFECTPARAM* pParam)
{
	...
	{
		if(pObject[n].pTarget->IsInited() == FALSE)	
			continue;
...
{
	if( pResultInfo->RealDamage > 10000000000 )
	{
		ASSERT(0);
		return;
	}
	...
	{
		LifeDamage = (DWORD)(LifeOriginalTotalDamage*CurRate);	
		ShieldDamage = (DWORD)(ShieldOriginalTotalDamage*CurRate);	
		VampiricLife = (DWORD)(VampiricLifeOriginalTotal*CurRate);	
		VampiricNaeryuk = (DWORD)(VampiricNaeryukOriginalTotal*CurRate);
	...
	if(SumRate >= 0.98f)
	{
		if(pVictim->IsDied())	
		{
			OBJECTACTIONMGR->Die(pVictim,pAttacker,bFatalDamage,pResultInfo->bCritical,pResultInfo->bDecisive);	
		}
	}
}

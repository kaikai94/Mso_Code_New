...
LONG CMiniDumper::unhandledExceptionHandler(_EXCEPTION_POINTERS *pExceptionInfo)
{
	if (!s_pMiniDumper)
	{
		::MessageBox(NULL,"!s_pMiniDumper","",MB_OK);
...
LONG CMiniDumper::writeMiniDump(_EXCEPTION_POINTERS *pExceptionInfo)
{
	...
	{
		::MessageBox(NULL,"getImpersonationToken","",MB_OK);
		return FALSE;
	}
	...
	if (GetModuleFileName(NULL, m_szAppPath, _MAX_PATH))
	{
		...
		TCHAR *pSlash = strchr(m_szAppPath, '\\');
		...
		//wcscat(szDbgHelpPath, "DBGHELP.DLL");
	...
	if (hDll)
	{
		...
		if (MiniDumpWriteDump != NULL)
		{
			...
			if (hFile != INVALID_HANDLE_VALUE)
			{
				...
				BOOL bOk;
				MessageBox(NULL,"游戏遇到问题异常退出，正在创建dmp","提示!",MB_OK);
				// DBGHELP.dll is not thread-safe, so we need to restrict access...
				MINIDUMP_TYPE mt = (MINIDUMP_TYPE)(MiniDumpWithDataSegs|MiniDumpWithFullMemory|MiniDumpWithHandleData|
					MiniDumpWithUnloadedModules|MiniDumpWithPrivateReadWriteMemory);

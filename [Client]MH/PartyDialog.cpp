...
#ifdef _XDYY_			
#include "cOnlineVoiceDlg.h"//[小队语音][By:十里坡剑神][QQ:112582793][2019/3/27][10:32:15]
#endif
//////////////////////////////////////////////////////////////////////
...
void CPartyDialog::SetActive(BOOL bState)
{
	...
	if(!bState)
	{
		if(GAMEIN->GetOnlineVoiceDlg()->IsActive())
			GAMEIN->GetOnlineVoiceDlg()->SetActive(FALSE);
	}
#endif
//--- main bar icon
...
#ifdef _XDYY_
//[小队语音][By:十里坡剑神][QQ:112582793][2019/3/21][17:03:29]
void CPartyDialog::OnActionEvent(LONG lId, void * p, DWORD we)
{
	for( int i = 0; i < MAX_PARTY_LISTNUM-1; ++i )
	{
		if(m_MemberDlg[i]->OnActionEvent(lId,p,we))
			break;
	}
}
void CPartyDialog::SetMemberVoiceBtn(DWORD memberID,BOOL bShow)
{
	for( int i = 0; i < MAX_PARTY_LISTNUM-1; ++i )
	{
		if(m_MemberDlg[i]->UpdateVoiceBtn(memberID,bShow))
			break;
	}
}
void CPartyDialog::SetMemberVoiceBtnRGB(DWORD memberID)
{
	for( int i = 0; i < MAX_PARTY_LISTNUM-1; ++i )
	{
		if(m_MemberDlg[i]->UpdateVoiceBtnRGB(memberID,RGBA_MAKE(0,250,0,255)))
			break;
	}
}
#endif

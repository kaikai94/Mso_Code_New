...
CQuestManager::CQuestManager()
{
	...
	//m_QuestStringTable.Initialize(1500);
	m_QuestTable.Initialize(MAX_QUEST);
	m_QuestNpcTable.Initialize(500);
...
void CQuestManager::Release()
{
	...
	m_dwLastMsg = 0;
	std::map<DWORD,CQuestString*>::iterator iter;
	iter = m_QuestStringTable.begin();
	while(1)
	{
		if(iter==m_QuestStringTable.end())
			break;
		else
		{
			delete iter->second;
	...
	while( !file.IsEOF() )
	{
		...
		if(strcmp(Token,"$SUBQUESTSTR") == 0)	
		{
			...
			if(pQuestString)
			{
				...
				m_QuestStringTable[Key] = pQuestString;
...
CQuestString* CQuestManager::GetQuestString(DWORD key)
{
	std::map<DWORD,CQuestString*>::iterator iter;
	iter = m_QuestStringTable.find(key);
	if(iter!=m_QuestStringTable.end())
		return iter->second;
	else
		return NULL;
	...
	else
	{
		...
		if( pQuest )
		{
			...
			if(pQuest->GetEndParam() == 0)
			{
				...
				m_QuestUpdater->ProcessQuestAdd(GetQuestString(Key));
			}
		}
	}
}
...
void CQuestManager::StartQuest( DWORD dwQuestIdx, DWORD dwSubQuestFlag, DWORD dwData, DWORD dwTime )
{
	...
	if( pQuest )
	{
		...
		m_QuestUpdater->ProcessQuestAdd( GetQuestString(Key));
		// title
		CQuestString* pQString = GetQuestString(Key);//m_QuestStringTable.GetData(Key);
		if( pQString == NULL )
			return;
...
void CQuestManager::EndQuest( DWORD dwQuestIdx, DWORD dwSubQuestFlag, DWORD dwData, DWORD dwTime )
{
	...
	if( pQuest )
	{
		...
		COMBINEKEY(dwQuestIdx, 0, Key);
		if(dwSubQuestFlag == 0)
			m_QuestUpdater->CompleteQuestDelete( GetQuestString(Key));//m_QuestUpdater->CompleteQuestDelete(m_QuestStringTable.GetData(Key));
...
void CQuestManager::EndSubQuest( DWORD dwQuestIdx, DWORD dwSubQuestIdx, DWORD dwSubQuestFlag, DWORD dwSubData, DWORD dwSubTime )
{
	...
	if( pQuest )
	{
		...
		m_QuestUpdater->ProcessQuestDelete(GetQuestString(Key));
	}
}
...
void CQuestManager::SendNpcTalkEvent( DWORD dwNpcIdx, DWORD dwNpcScriptIdx,DWORD dwNpcID )
{
	SEND_QUEST_IDX Msg;

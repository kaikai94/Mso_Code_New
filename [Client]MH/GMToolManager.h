...
public:
	...
	void DisplayLocalInfo(char* LocalInfo);

	void SetCanGetItem(BOOL bVal){m_CanGetItem = bVal;}
	BOOL IsCanGetItem(){return m_CanGetItem;}
	eGETITEMMODE GetMode(){return m_GetMode;}
	void SetGetMode(eGETITEMMODE Val){m_GetMode = Val;}

	void SetItemTip(char * message);
	void ChangeObject(char * name);

	void SendGameConfig();
	void UpdateGameConfig(MSG_GAMECFG * msg);
	void SetCanSend(BOOL val){m_CanSendCfg = val;}
	BOOL IsCanSend(){return m_CanSendCfg;}
};

...
bool MD5_CTX::GetFileMd5(std::wstring& pMd5, const char *pFileName)
{
	TCHAR szmd5buf[16] = { 0 };
	...
	unsigned char *pInPut = new unsigned char[length];
	...
	fclose(pFile);
	//static std::string szmd5/*[UPDATE_MD5_LEN*2+1] = {0}*/;
	//szmd5.empty();

	for (int i = 0; i < UPDATE_MD5_LEN; i++)
	{
		_itow_s(chDigest[i], szmd5buf, 16, 16);
		if (0 == szmd5buf[1])
		{
			pMd5 += L"0";
			pMd5 += szmd5buf;
			//strcat_s(szmd5, "0");
			//strcat_s(szmd5, szmd5buf);
		}
		else
		{
			pMd5 += szmd5buf;
	...
	transform(pMd5.begin(), pMd5.end(), pMd5.begin(), toupper);
	//_strupr_s(szmd5);
	//pMd5 = szmd5;
	return true;
}
std::wstring MD5_CTX::stringToWstring(const std::string& str)
{

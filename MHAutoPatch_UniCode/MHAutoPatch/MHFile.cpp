...
int CMHFile::GetString( char* pBuf )
{
	...
	int rt = 0;
			...
			if(m_Dfp == (int)m_Header.dwFileSize) return (rt=EOF);
...
BOOL CMHFile::IsEOF()
{
	if (m_Header.dwFileSize == (DWORD)m_Dfp)
		return TRUE;
	else
		return FALSE;
}
...
std::wstring CMHFile::GetStringW()
{
	std::wstring buf;
	GetString(buf);
	return buf;
}
int CMHFile::GetString(std::wstring&pBuf)
{
	char buf[512] = { 0, };
	std::string str;
	DWORD j = 0;
	...
	buf[j] = 0;
	str = buf;
	pBuf = stringToWstring(str);

	return rt;
}
std::wstring CMHFile::stringToWstring(const std::string& str)
{

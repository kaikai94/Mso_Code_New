...
void __stdcall CMSSystem::Process( DWORD dwEvent )
{
...
//				ASSERT(pInfo->wServerState != SERVERSTATE_OFF);
				...
				if( tick - pInfo->dwExpiredTime > 10000)
				{
					...
					MNETWORK->SendToMapServer(pInfo->dwConnectionIndex, (char *)&msg, sizeof(MSG_BYTE));
				}
...
void __stdcall CMSSystem::OnConnectMASSuccess(DWORD dwConnectionIndex, void* pVoid)
{
...
void __stdcall CMSSystem::OnConnectMASFail(void* pVoid)
{
...
//	}
	...
	yCASE(MP_RMTOOL_ADMIN)
	{
		if( IPCHECKMGR->GetPower( dwConnectionIndex ) != eOperPower_Master )
			return;
...
void CMSSystem::UserMsgParse(DWORD dwConnectionIndex,char* pMsg,DWORD dwLength)
{
	...
	ySWITCH(pTempMsg->Protocol)
	{
		...
		yCASE(MP_RMTOOL_SIMPLECHARACTER_SYN)
		{
			...
			CharacterCreateInfoFromMN( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->dwMemberNo, 0 );
		}
...
void CMSSystem::MunpaMsgParse(DWORD dwConnectionIndex,char* pMsg,DWORD dwLength)
{
	...
	ySWITCH(pTempMsg->Protocol)
	{
		yCASE(MP_RMTOOL_GUILD_SYN)
		{
			TMSGBASE* pmsg = (TMSGBASE*)pMsg;
			SearchGuild( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, 0 );
		}
		yCASE(MP_RMTOOL_GUILDIFNO_SYN)
		{
			TMSG_MUNPASEARCH* pmsg = (TMSG_MUNPASEARCH*)pMsg;
			SearchGuildInfo( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->strSearch );
		}
		...
		yCASE(MP_RMTOOL_MUNPAINFO_RESYN)
		{
			...
			MunpaBaseInfo((WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->dwData);
		}
		yCASE(MP_RMTOOL_MUNPAMONEYUPDATE)
		{
			MSG_GUILDMONEYUPDATE* msg = (MSG_GUILDMONEYUPDATE*)pMsg;

			MunpaMoneyUpdate( msg->GuildIdx, msg->Money );
		}
	}
	yENDSWITCH	
}
...
void CMSSystem::GameLogMsgParse(DWORD dwConnectionIndex,char* pMsg,DWORD dwLength)
{
	...
	ySWITCH(pTempMsg->Protocol)
	{
		...
		/// 060831 PKH 문파토너먼트
		yCASE(MP_RMTOOL_QUERY_GUILDTNIDX_SYN)
		{
			TMSG_CHECKNAME* pmsg = (TMSG_CHECKNAME*)pMsg;
			SearchGuildIdxFromGuildName( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->sName );
		}
		/// 060831 PKH 문파토너먼트
		yCASE(MP_RMTOOL_QUERY_SIEGEWARIDX_SYN)
		{
			TMSG_CHECKNAME* pmsg = (TMSG_CHECKNAME*)pMsg;
			SearchSWGuildIdxFromGuildName( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->sName );
		}
		...
		yCASE(MP_RMTOOL_PETLOGFROMTYPE_SYN)
		{
			TMSG_GAMELOGSEARCH* pmsg = (TMSG_GAMELOGSEARCH*)pMsg;

			PetLogFromType( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, 0, pmsg->wType,
								  pmsg->dwChrIdx, pmsg->dwItem, pmsg->bDayOnly, pmsg->sSDate, pmsg->sEDate ); 	
		}	
		...
		/// 060831 PKH 문파토너먼트 조회
		yCASE(MP_RMTOOL_GUILDTNLOGFROMTYPE_SYN)
		{
			TMSG_GAMELOGSEARCH* pmsg = (TMSG_GAMELOGSEARCH*)pMsg;

			GuildTNLogFromType( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, 0, pmsg->wType,
								  pmsg->dwChrIdx, pmsg->bDayOnly, pmsg->sSDate, pmsg->sEDate ); 	
		}	
		/// 060831 PKH 공성전
		yCASE(MP_RMTOOL_SIEGEWARLOGFROMTYPE_SYN)
		{
			TMSG_GAMELOGSEARCH* pmsg = (TMSG_GAMELOGSEARCH*)pMsg;

			SiegeWarLogFromType( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, 0, pmsg->wType,
								  pmsg->dwChrIdx, pmsg->bDayOnly, pmsg->sSDate, pmsg->sEDate ); 	
		}	
		/// 060915 PKH GMTOOL
		yCASE(MP_RMTOOL_GMTOOLLOGFROMTYPE_SYN)
		{
			TMSG_GAMELOGSEARCH* pmsg = (TMSG_GAMELOGSEARCH*)pMsg;

			GMToolLogFromType( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, 0, pmsg->wType,
				pmsg->dwChrIdx, pmsg->dwItem, pmsg->bDayOnly, pmsg->sSDate, pmsg->sEDate ); 	
		}
		yCASE(MP_RMTOOL_ITEMOPTIONLOG_SYN)
		{
			TMSG_GAMELOGSEARCH* pmsg = (TMSG_GAMELOGSEARCH*)pMsg;
			if( pmsg->wServer == 0 )
			{
				ItemOptionLog( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, 0, pmsg->dwItem, pmsg->bDayOnly, pmsg->sSDate, pmsg->sEDate );
			}
			else
			{
				ItemOptionLog2( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, 0, pmsg->dwItem, pmsg->bDayOnly, pmsg->sSDate, pmsg->sEDate );
			}
		}
	}
	yENDSWITCH	
...
void CMSSystem::AdminMsgParse(DWORD dwConnectionIndex,char* pMsg,DWORD dwLength)
{
	...
	ySWITCH(pTempMsg->Protocol)
	{
		yCASE(MP_RMTOOL_ADMIN_TOTALINFO_SYN)
		{
			...
			TotalGMIpAdressInfo((WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, 0);
		...
		/// 060829 PKH - gm툴 허용 아이피 등록
		yCASE(MP_RMTOOL_ADMIN_INSERTGMIP_SYN)
		{
			TMSG_ADMIN_INSERTIP* msg = (TMSG_ADMIN_INSERTIP*)pMsg; 
			InsertGMIpAdress((WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->strIP);

			// log
			InsertLogTool( eToolLog_RM, eRMLog_GMIpInsert, pData->dwIdx, pData->sID, 0, msg->strIP, 0, 0, 0, 0 );
		}	
		/// 060829 PKH - gm툴 허용 아이피 삭제
		yCASE(MP_RMTOOL_ADMIN_DELETEGMIP_SYN)
		{
			TMSG_DWORD* pmsg = (TMSG_DWORD*)pMsg;
			DeleteGMIpAdress( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->dwData );

			// log
			InsertLogTool( eToolLog_RM, eRMLog_GMIpDelete, pData->dwIdx, pData->sID, pmsg->dwData, "", 0, 0, 0, 0 );
		}
	}
	yENDSWITCH	
}
...
void CMSSystem::CharacterMsgParse(DWORD dwConnectionIndex,char* pMsg,DWORD dwLength)
{
	...
	ySWITCH(pTempMsg->Protocol)
	{
		yCASE(MP_RMTOOL_CHARACTERINFO_SYN)
		{
			...
			CharacterInvenInfo2( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, 0, msg->dwData3 );
			...
			CharacterShopWareItemInfo( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->dwData2 );

//#ifndef _CHINA_LOCAL_
			CharacterRareItemOptionInfo( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->dwData2, msg->dwData3, 0 );
//#endif
...
//			CharacterInvenInfo((WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->dwData3);
		...
		{
			WORD power = IPCHECKMGR->GetPower( dwConnectionIndex );
			if( power != eOperPower_Master && power != eOperPower_QA )
				return;
...
//			InsertLogTool( eToolLog_RM, eRMLog_ChangeSkillPosition, pData->dwIdx, pData->sID, msg->wPower, msg->strID, 0, 0, 0 );
		...
		yCASE(MP_RMTOOL_CHARACTER_USINGSHOPITEMINFO_SYN)
		{
			TMSG_DWORD* pmsg = (TMSG_DWORD*)pMsg;
			CharacterUsingShopItemInfo( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->dwData );
		}
		yCASE(MP_RMTOOL_CHARACTER_USINGSHOPITEMINFO_DELETE_SYN)
		{
			WORD power = IPCHECKMGR->GetPower( dwConnectionIndex );
			if( power != eOperPower_Master && power != eOperPower_QA )
				return;
			...
			TMSG_DWORD3* pmsg = (TMSG_DWORD3*)pMsg;
			CharacterUsingShopItemInfoDelete( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->dwData1, pmsg->dwData2, pmsg->dwData3 );			
		}
		yCASE(MP_RMTOOL_CHARACTER_USINGSHOPITEMINFO_EDIT_SYN)
		{
			WORD power = IPCHECKMGR->GetPower( dwConnectionIndex );
			if( power != eOperPower_Master && power != eOperPower_QA )
				return;
			...
			TMSG_DWORD3* pmsg = (TMSG_DWORD3*)pMsg;
			CharacterUsingShopItemInfoEdit( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->dwData1, pmsg->dwData2, pmsg->dwData3 );			
		}
		yCASE(MP_RMTOOL_CHARACTER_MAINPETLIST_SYN)
		{
			TMSG_DWORD* pmsg = (TMSG_DWORD*)pMsg;
			CharacterMainPetList( (WORD)dwConnectionIndex, (WORD)pmsg->dwTemplateIdx, pmsg->dwData );
		}
	}
	yENDSWITCH	
}
...
void CMSSystem::ItemMsgParse(DWORD dwConnectionIndex,char* pMsg,DWORD dwLength)
{
	...
	ySWITCH(pTempMsg->Protocol)
	{
		yCASE(MP_RMTOOL_INSERTITEM)
		{
			...
			{
				if ( msg->ItemKind == eTITAN_ITEM_PAPER )
					InvenItemInsert( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->sCharName, msg->dwTagetIdx, &msg->Item, &msg->Option, msg->dwUserIdx, msg->ItemKind, &msg->TitanInfo );
				else
					InvenItemInsert( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->sCharName, msg->dwTagetIdx, &msg->Item, &msg->Option );
			...
			{
				PyogukItemInsert( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->dwTagetIdx, &msg->Item, &msg->Option);
			}
			else if(msg->bTable == 2) // Munpa
			{
				MunpaItemInsert( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->dwTagetIdx, &msg->Item, &msg->Option);
			}
			else if(msg->bTable == 3 )
			{
				ShopWareItemInsert( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->dwTagetIdx, &msg->Item, &msg->Option );
			}

			else if(msg->bTable == 4) // pet
			{
				InvenItemInsert( (WORD)dwConnectionIndex, (WORD)msg->dwTemplateIdx, msg->sCharName, msg->dwTagetIdx, &msg->Item, &msg->Option);

			/*	if( TP_PETINVEN_START <= msg->Item.Position && msg->Item.Position < TP_PETINVEN_START )
				{
					BYTE WearType = msg->Item.Position - TP_WEAR_START;
...
//pet 
void CMSSystem::PetMsgParse(DWORD dwConnectionIndex,char* pMsg,DWORD dwLength)
{
...
//삭제된 케릭터 조회
void CMSSystem::DelCharMsgParse(DWORD dwConnectionIndex,char* pMsg,DWORD dwLength)
{
	MSGROOT* pTempMsg = (MSGROOT*)pMsg;
	ySWITCH(pTempMsg->Protocol)
	{

...
void RTotalIpAdressInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	{
		Msg.sIP[Msg.wCount].dwIPIdx = (DWORD)atoi((char*)pData[i].Data[eII_IPIdx]);
...
void RTotalOperatorInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	DWORD MyIdx = (DWORD)atoi((char*)pData[0].Data[eOI_MyIdx]);
	DWORD startIdx = 0;
	Msg.wCount = 0;

	for( DWORD i = 0 ; i < pMessage->dwResult ; ++i )
	{
		Msg.sOper[Msg.wCount].dwOperIdx = (DWORD)atoi((char*)pData[i].Data[eOI_OperIdx]);
...
void ROperatorLoginCheck(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	Msg.sOper.dwOperIdx = (DWORD)atoi((char*)pData[0].Data[eOI_OperIdx]);
	...
	Msg.sIP.dwIPIdx = (DWORD)atoi((char*)pData[0].Data[eOI_IPIdx]);
	...
	IPCHECKMGR->RegistID( connectIdx, Msg.sOper.dwOperIdx, Msg.sOper.strOperID, Msg.sOper.wOperPower );
}
...
void RSearchUserInfo(LPMIDDLEQUERY pData, LPDBMESSAGE pMessage)
{
	...
	else
	{
		...
		Info.sUser.dwMemberNo = (DWORD)atoi((char*)pData[0].Data[eUI_User_Idx]);
...
void RCharacterBaseInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	Msg.m_sObjinfo.dwObjectID = (DWORD)atoi((char*)pData->Data[eCS_CharacterIdx]);
	Msg.m_sObjinfo.dwUserID =  (DWORD)atoi((char*)pData->Data[eCS_UserIdx]);
	strcpy( Msg.m_sObjinfo.ObjectName, (char*)pData->Data[eCS_Name] );
	
	Msg.m_sTotalinfo.Gender = atoi((char*)pData->Data[eCS_Gender]);
	Msg.m_sTotalinfo.Life = (DWORD)atoi((char*)pData->Data[eCS_Life]);
	Msg.m_sTotalinfo.Shield = (DWORD)atoi((char*)pData->Data[eCS_Shield]);
	...
	Msg.m_sTotalinfo.MunpaID = (DWORD)atoi((char*)pData->Data[eCS_MunpaIdx]);
	...
	Msg.m_sTotalinfo.MaxLife = (DWORD)atoi((char*)pData->Data[eCS_MaxLife]);

	Msg.m_sHeroinfo.AbilityExp = (DWORD)atoi((char*)pData->Data[eCS_AbilityExp]);

	// magi82 EXP Type Change -> 경험치는 INT64로 바뀌었으므로 atoi함수도 INT64버전용을 써야한다.
	Msg.m_sHeroinfo.ExpPoint = (EXPTYPE)_atoi64((char*)pData->Data[eCS_Expoint]);
	...
	Msg.m_sHeroinfo.Money = (DWORD)atoi((char*)pData->Data[eCS_Money]);
	strcpy( Msg.m_sTotalinfo.GuildName, (char*)pData->Data[eCS_MunpaName] );
	Msg.m_sHeroinfo.PartyID = (DWORD)atoi((char*)pData->Data[eCS_PartyIdx]);
...
{
	sprintf( txt, "EXEC RP_UpdateCharacterInfo %d, %I64d, %d, %d, %d, %d, %d, %d, %d, %d, %d",
...
void RCharacterPyogukInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	{
		UserIdx = (DWORD)atoi((char*)pData->Data[eCP_UserIdx]);
		Msg.m_wPyogukNum = atoi((char*)pData->Data[eCP_PyogukNum]);
		Msg.m_dwMoney = (DWORD)atoi((char*)pData->Data[eCP_PyogukMoney]);
	}
...
void RCharacterPyogukItemInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	DWORD UserIdx = (DWORD)atoi((char*)pData[0].Data[ePI_UserId]);
	POSTYPE startPos = 0;
	Msg.m_wCount = 0;
	...
	for( DWORD i = 0 ; i < pMessage->dwResult ; ++i )
	{
		...
		{
			Msg.m_Item[Msg.m_wCount].dwDBIdx			= (DWORD)atoi((char*)pData[i].Data[ePI_DBIDX]);
			...
			Msg.m_Item[Msg.m_wCount].Durability		= (DWORD)atoi((char*)pData[i].Data[ePI_Durability]);
			Msg.m_Item[Msg.m_wCount].RareIdx = (DWORD)atoi((char*)pData[i].Data[ePI_Durability+1]);
...
void RCharacterMugongInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	for(DWORD i = 0; i < count; i++)
	{
		...
		POSTYPE MugongPos = atoi((char*)pData[i].Data[eCM_Position]) - 555;
#else
		...
		if( Msg.m_sMugongInfo.mugong[MugongPos].dwDBIdx == 0 )
		{
			...
			Msg.m_sMugongInfo.mugong[MugongPos].dwDBIdx = (DWORD)atoi((char*)pData[i].Data[eCM_DBIDX]);
			Msg.m_sMugongInfo.mugong[MugongPos].ExpPoint = (DWORD)atoi((char*)pData[i].Data[eCM_ExpPoint]);
		...
		else
		{
			if( ec < 5 )
			{
				...
				Msg2.m_MugongBase[ec].dwDBIdx = (DWORD)atoi((char*)pData[i].Data[eCM_DBIDX]);
				Msg2.m_MugongBase[ec].ExpPoint = (DWORD)atoi((char*)pData[i].Data[eCM_ExpPoint]);
...
void RCharacterItemOptionInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	DWORD UserIdx = (DWORD)atoi((char*)pData[0].Data[eCIOI_UserID]);
	DWORD CharIdx = (DWORD)atoi((char*)pData[0].Data[eCIOI_CharID]);
	DWORD StartIdx = 0;
...
void RSearchMunpaIdx(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	{
		DWORD dwMunpaIdx = (DWORD)atoi((char*)pData[0].Data[0]);
...
void RMunpaBaseInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	else
	{
		...
		Info.sBaseInfo.dwFamous = (DWORD)atoi((char*)pData[0].Data[eMB_Famous]);
		Info.sBaseInfo.bKind = atoi((char*)pData[0].Data[eMB_Kind]);
		Info.sBaseInfo.dwMoney = (DWORD)atoi((char*)pData[0].Data[eMB_MunpaMoney]);
...
void RMunpaMemberInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	DWORD MunpaIdx = (DWORD)atoi((char*)pData[0].Data[eMM_MunpaId]);
	DWORD startIdx = 0;
	Msg.wCount = 0;

	for( DWORD i = 0 ; i < pMessage->dwResult ; ++i )
	{
		Msg.sMember[Msg.wCount].dwCharno		= (DWORD)atoi((char*)pData[i].Data[eMM_MemberIdx]);
...
{
	sprintf(txt, "EXEC RP_MunpaMoneyUpdate %d, %d", GuildIdx, Money);	
	if(g_DB.Query(eQueryType_FreeQuery, eMunpaMoneyUpdate, 0, txt) == FALSE)
	{
	}
}
...
void RGameLogItemMoveLog(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	for( DWORD i = 0 ; i < pMessage->dwResult ; ++i )
	{
		Msg.sLog[Msg.wCount].dwLogidx = (DWORD)atoi((char*)pData[i].Data[eGI_LogIdx]);
		Msg.sLog[Msg.wCount].bLogtype = atoi((char*)pData[i].Data[eGI_LogType]);
		Msg.sLog[Msg.wCount].dwCharIdx = (DWORD)atoi((char*)pData[i].Data[eGI_CharIdx]);
		Msg.sLog[Msg.wCount].dwTargetIdx = (DWORD)atoi((char*)pData[i].Data[eGI_TargetIdx]); 
		strcpy(Msg.sLog[Msg.wCount].strTargetName, (char*)pData[i].Data[eGI_TargetName]);
		Msg.sLog[Msg.wCount].dwItemIdx = (DWORD)atoi((char*)pData[i].Data[eGI_ItemIdx]);
		Msg.sLog[Msg.wCount].dwItemDBIdx = (DWORD)atoi((char*)pData[i].Data[eGI_ItemDBIdx]);
		...
		Msg.sLog[Msg.wCount].dwMoney = (DWORD)atoi((char*)pData[i].Data[eGI_Money]);
...
void RGameLogMoneyLog(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	for( DWORD i = 0 ; i < pMessage->dwResult ; ++i )
	{
		Msg.sLog[Msg.wCount].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[eGLM_LogIdx]);
		Msg.sLog[Msg.wCount].LogType = atoi((char*)pData[i].Data[eGLM_LogType]);
		Msg.sLog[Msg.wCount].dwCharIdx = (DWORD)atoi((char*)pData[i].Data[eGLM_CharIdx]);
		Msg.sLog[Msg.wCount].dwInvenMoney = (DWORD)atoi((char*)pData[i].Data[eGLM_InvenMoney]);
		Msg.sLog[Msg.wCount].dwPyogukMoney = (DWORD)atoi((char*)pData[i].Data[eGLM_PyogukMoney]);
...
//		msg.Protocol = MP_RMTOOL_GAMELOG_ITEM_NACK;
	...
	Msg.dwItemDBIdx = (DWORD)atoi((char*)pData[0].Data[eGLI_ChrName]);
	Msg.bDayOnly = atoi((char*)pData[0].Data[eGLI_DayOnly]);
	strcpy( Msg.strStart, (char*)pData[0].Data[eGLI_StartDate] );
	strcpy( Msg.strEnd, (char*)pData[0].Data[eGLI_EndDate] );
	Msg.wCount = 0;
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[eGLI_LogIdx]);
		Msg.sLog[i].dwLogType = (DWORD)atoi((char*)pData[i].Data[eGLI_LogType]);
		Msg.sLog[i].dwItemIdx = (DWORD)atoi((char*)pData[i].Data[eGLI_ItemIdx]);
		Msg.sLog[i].dwItemDBIdx = (DWORD)atoi((char*)pData[i].Data[eGLI_ItemDBIdx]);
		Msg.sLog[i].dwCurChrIdx = (DWORD)atoi((char*)pData[i].Data[eGLI_CurChrIdx]);
		Msg.sLog[i].dwBeforeChrIdx = (DWORD)atoi((char*)pData[i].Data[eGLI_BeforeChrIdx]);
		Msg.sLog[i].dwMoney = (DWORD)atoi((char*)pData[i].Data[eGLI_Money]);
		strcpy( Msg.sLog[i].strLogDate, (char*)pData[i].Data[eGLI_LogDate] );
		strcat( Msg.sLog[i].strLogDate, " ");
		strcat( Msg.sLog[i].strLogDate, (char*)pData[i].Data[eGLI_LogTime] );
		strcpy( Msg.sLog[i].strCurChrName, (char*)pData[i].Data[eGLI_CurChrName] );
...
void InvenItemInsert( WORD wConnectIdx, WORD wTempIdx, char* sCharName, DWORD dwCharIdx, ITEMBASE* pItem, ITEM_OPTION_INFO* pOptionInfo, DWORD dwUserIdx /*= 0*/, DWORD dwItemKind /*= 0*/, TITAN_TOTALINFO * pTitanTotalInfo /*= NULL*/ )
{
	...
	sprintf(buf, "\'%s\', %d, %d, %d, %d, %d,   %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d",
		sCharName,
		dwCharIdx, 
		pItem->wIconIdx,
		pItem->Durability,
	...
	if ( dwItemKind != eTITAN_ITEM_PAPER )
	{
		sprintf( txt, "%s %s", "EXEC RP_InvenItemInsert", buf );
	}
	else
	{
		sprintf( txt, "%s %s, %d,%d,%d,%d,%d, %d,%d,%d,%d,%d, %d,%d", "EXEC RP_InvenTitanItemInsert ", buf,
			dwUserIdx,
			pItem->wIconIdx,
...
void RInvenItemInsert(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	if( pMessage->dwResult == 1 )
	{
		...
		item.Durability = (DWORD)atoi((char*)pData[0].Data[6]);
...
void RTotalIpInfo( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < count; ++i )
	{
		...
		startIdx = (DWORD)atoi((char*)pData[i].Data[eII_IPIdx]);
	}
...
void RItemMoneyLogFromType( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	Msg.dwCharIdx = (DWORD)atoi((char*)pData[0].Data[eIML_ChrName]);
	Msg.dwItem = (DWORD)atoi((char*)pData[0].Data[eIML_Item]);
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[eIML_LogIdx]);
		Msg.sLog[i].dwLogType = (DWORD)atoi((char*)pData[i].Data[eIML_LogType]);
		strcpy( Msg.sLog[i].sFromChrName, (char*)pData[i].Data[eIML_FromChrName] );
		Msg.sLog[i].dwFromChrIdx = (DWORD)atoi((char*)pData[i].Data[eIML_FromChrIdx]);		
		strcpy( Msg.sLog[i].sToChrName, (char*)pData[i].Data[eIML_ToChrName] );
		Msg.sLog[i].dwToChrIdx = (DWORD)atoi((char*)pData[i].Data[eIML_ToChrIdx]);		
		Msg.sLog[i].dwFromTotalMoney = (DWORD)atoi((char*)pData[i].Data[eIML_FromTotalMoney]);
		Msg.sLog[i].dwToTotalMoney = (DWORD)atoi((char*)pData[i].Data[eIML_ToTotalMoney]);
		Msg.sLog[i].dwChangeMoney = (DWORD)atoi((char*)pData[i].Data[eIML_ChangeMoney]);
		Msg.sLog[i].dwItemIdx = (DWORD)atoi((char*)pData[i].Data[eIML_ItemIdx]);
		Msg.sLog[i].dwItemDBIdx = (DWORD)atoi((char*)pData[i].Data[eIML_ItemDBIdx]);
		Msg.sLog[i].dwItemDur = (DWORD)atoi((char*)pData[i].Data[eIML_ItemDur]);
		Msg.sLog[i].dwItemFromPosition = (DWORD)atoi((char*)pData[i].Data[eIML_ItemFromPosition]);
		Msg.sLog[i].dwItemToPosition = (DWORD)atoi((char*)pData[i].Data[eIML_ItemToPosition]);
...
void RCharacterLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	Msg.dwCharIdx = (DWORD)atoi((char*)pData[0].Data[0]);
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[4]);
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[5]);		
		Msg.sLog[i].dwGrade = (DWORD)atoi((char*)pData[i].Data[6]);
		Msg.sLog[i].dwGengoal = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwDex = (DWORD)atoi((char*)pData[i].Data[8]);
		Msg.sLog[i].dwStrength = (DWORD)atoi((char*)pData[i].Data[9]);
		Msg.sLog[i].dwSimmak = (DWORD)atoi((char*)pData[i].Data[10]);
		Msg.sLog[i].Point = _atoi64((char*)pData[i].Data[11]);
...
void RExpPointLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[4]);
		Msg.sLog[i].dwLogType = (DWORD)atoi((char*)pData[i].Data[5]);		
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[6]);		
		Msg.sLog[i].dwGrade = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwChangeValue = (DWORD)atoi((char*)pData[i].Data[8]);
		Msg.sLog[i].dwKillerKind = (DWORD)atoi((char*)pData[i].Data[9]);
		Msg.sLog[i].dwKillerIdx = (DWORD)atoi((char*)pData[i].Data[10]);
		Msg.sLog[i].ExpPoint = _atoi64((char*)pData[i].Data[11]);
		Msg.sLog[i].dwAbilPoint = (DWORD)atoi((char*)pData[i].Data[12]);
...
void RMugongLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		...
		Msg.sLog[i].dwLogType = (DWORD)atoi((char*)pData[i].Data[5]);		
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[6]);
		Msg.sLog[i].dwSkillIdx = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwSkillDBIdx = (DWORD)atoi((char*)pData[i].Data[8]);
		Msg.sLog[i].dwSkillLevel = (DWORD)atoi((char*)pData[i].Data[9]);
		Msg.sLog[i].dwSkillExp = (DWORD)atoi((char*)pData[i].Data[10]);
...
void RMugongExpLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[4]);
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[5]);		
		Msg.sLog[i].dwChangeValue = (DWORD)atoi((char*)pData[i].Data[6]);
		Msg.sLog[i].dwSkillIdx = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwSkillDBIdx = (DWORD)atoi((char*)pData[i].Data[8]);
		Msg.sLog[i].dwSkillLevel = (DWORD)atoi((char*)pData[i].Data[9]);
		Msg.sLog[i].dwSkillExp = (DWORD)atoi((char*)pData[i].Data[10]);
		Msg.sLog[i].dwSkillPos = (DWORD)atoi((char*)pData[i].Data[11]);
		strcpy( Msg.sLog[i].sLogDate, (char*)pData[i].Data[12] );
		strcat( Msg.sLog[i].sLogDate, " ");
		strcat( Msg.sLog[i].sLogDate, (char*)pData[i].Data[13]);
...
void RMoneyLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		...
		Msg.sLog[i].dwLogType = (DWORD)atoi((char*)pData[i].Data[5]);
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[6]);					
		Msg.sLog[i].dwChangeValue = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwInvenMoney = (DWORD)atoi((char*)pData[i].Data[8]);
		Msg.sLog[i].dwTargetIdx = (DWORD)atoi((char*)pData[i].Data[9]);
		Msg.sLog[i].dwTargetMoney = (DWORD)atoi((char*)pData[i].Data[10]);
		strcpy( Msg.sLog[i].sLogDate, (char*)pData[i].Data[11] );
		strcat( Msg.sLog[i].sLogDate, " ");
		strcat( Msg.sLog[i].sLogDate, (char*)pData[i].Data[12]);
...
void RMoneyWrongLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		...
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[6]);		
		Msg.sLog[i].dwChangeValue = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwTotalMoney = (DWORD)atoi((char*)pData[i].Data[8]);
		Msg.sLog[i].dwPyogukMoney = (DWORD)atoi((char*)pData[i].Data[9]);
		Msg.sLog[i].dwTargetIdx = (DWORD)atoi((char*)pData[i].Data[10]);
		strcpy( Msg.sLog[i].sLogDate, (char*)pData[i].Data[11] );
		strcat( Msg.sLog[i].sLogDate, " ");
		strcat( Msg.sLog[i].sLogDate, (char*)pData[i].Data[12]);
...
void RAbilityLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[4]);
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[5]);
		Msg.sLog[i].dwAbilityIdx = (DWORD)atoi((char*)pData[i].Data[6]);		
		Msg.sLog[i].dwAbilityLevelTo = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwAbilityExpFrom = (DWORD)atoi((char*)pData[i].Data[8]);
		Msg.sLog[i].dwAbilityExpTo = (DWORD)atoi((char*)pData[i].Data[9]);
...
void RItemOptionLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[4]);
		Msg.sLog[i].dwOptionDBIdx = (DWORD)atoi((char*)pData[i].Data[5]);
		Msg.sLog[i].dwChrIdx =(DWORD) atoi((char*)pData[i].Data[6]);
		Msg.sLog[i].dwItemDBIdx = (DWORD)atoi((char*)pData[i].Data[7]);
...
void RCheatLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[4]);
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[5]);
		Msg.sLog[i].dwLogType = (DWORD)atoi((char*)pData[i].Data[6]);		
		Msg.sLog[i].dwParam = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwItemDBIdx = (DWORD)atoi((char*)pData[i].Data[8]);
		Msg.sLog[i].dwItemIdx = (DWORD)atoi((char*)pData[i].Data[9]);
		strcpy( Msg.sLog[i].sLogDate, (char*)pData[i].Data[10] );
		strcat( Msg.sLog[i].sLogDate, " ");
		strcat( Msg.sLog[i].sLogDate, (char*)pData[i].Data[11]);
...
void RHackingLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[4]);
		Msg.sLog[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[5]);
		Msg.sLog[i].dwCheatKind = (DWORD)atoi((char*)pData[i].Data[6]);
...
void RToolLog( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	Msg.dwTemplateIdx = tempIdx;
	Msg.wCount = (WORD)pMessage->dwResult;
	Msg.dwLogType = (DWORD)atoi((char*)pData[0].Data[0]);
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwLogIdx = (DWORD)atoi((char*)pData[i].Data[5]);
		Msg.sLog[i].dwLogType = (DWORD)atoi((char*)pData[i].Data[6]);
		Msg.sLog[i].dwLogKind = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.sLog[i].dwOperIdx = (DWORD)atoi((char*)pData[i].Data[8]);
		strncpy( Msg.sLog[i].sOperName, (char*)pData[i].Data[9], 17 );
		Msg.sLog[i].dwTargetIdx = (DWORD)atoi((char*)pData[i].Data[10]);
		strncpy( Msg.sLog[i].sTargetName, (char*)pData[i].Data[11], 17 );
		Msg.sLog[i].dwParam1 = (DWORD)atoi((char*)pData[i].Data[12]);
		Msg.sLog[i].dwParam2 = (DWORD)atoi((char*)pData[i].Data[13]);
		Msg.sLog[i].dwParam3 = (DWORD)atoi((char*)pData[i].Data[14]);
		Msg.sLog[i].dwParam4 = (DWORD)atoi((char*)pData[i].Data[15]);
...
void RPetLogFromType( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	Msg.dwCharIdx = (DWORD)atoi((char*)pData[0].Data[ePL_ChrName]);
	Msg.dwItem = (DWORD)atoi((char*)pData[0].Data[ePL_Item]);
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.sLog[i].dwIdx = (DWORD)atoi((char*)pData[i].Data[ePL_Idx]);
		Msg.sLog[i].dwLogType = (DWORD)atoi((char*)pData[i].Data[ePL_LogType]);
		Msg.sLog[i].dwPetidx = (DWORD)atoi((char*)pData[i].Data[ePL_PetIdx]);		
		Msg.sLog[i].dwPetSummonItemDBIdx = (DWORD)atoi((char*)pData[i].Data[ePL_PetSummonItemDBIdx]);
		Msg.sLog[i].dwUserIdx = (DWORD)atoi((char*)pData[i].Data[ePL_UserIdx]);		
		Msg.sLog[i].dwCharacterIdx = (DWORD)atoi((char*)pData[i].Data[ePL_CharacterIdx]);
...
void RGuildTNLogFromType( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	Msg.guildidx = (DWORD)atoi((char*)pData[0].Data[1]);
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.logdata[i].idx = (DWORD)atoi((char*)pData[i].Data[5]);
		Msg.logdata[i].tournamentidx = (DWORD)atoi((char*)pData[i].Data[6]);
		Msg.logdata[i].guildidx = (DWORD)atoi((char*)pData[i].Data[7]);		
...
void RSiegeWarLogFromType( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	{
		Msg.logdata[i].idx = (DWORD)atoi((char*)pData[i].Data[5]);
		Msg.logdata[i].siegewaridx = (DWORD)atoi((char*)pData[i].Data[6]);
		Msg.logdata[i].mapnum = atoi((char*)pData[i].Data[7]);
		Msg.logdata[i].guildidx = (DWORD)atoi((char*)pData[i].Data[8]);		
		Msg.logdata[i].logkind = atoi((char*)pData[i].Data[9]);
		Msg.logdata[i].engraveidx = (DWORD)atoi((char*)pData[i].Data[10]);		
...
void RGMToolLogFromType( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	Msg.count = 0;
	Msg.type = atoi((char*)pData[0].Data[0]);
	Msg.character_idx = (DWORD)atoi((char*)pData[0].Data[1]);
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		Msg.logdata[i].dbidx = (DWORD)atoi((char*)pData[i].Data[6]);
		Msg.logdata[i].character_idx = (DWORD)atoi((char*)pData[i].Data[7]);
		Msg.logdata[i].gmlogtype = atoi((char*)pData[i].Data[8]);
		Msg.logdata[i].logkind = (DWORD)atoi((char*)pData[i].Data[9]);		
		Msg.logdata[i].param1 = (DWORD)atoi((char*)pData[i].Data[10]);
		Msg.logdata[i].param2 = (DWORD)atoi((char*)pData[i].Data[11]);		
...
void RSimpleCharacterInfoFromMN( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	else
	{
		...
		for( DWORD i = 0; i < count; ++i )
		{
			Msg.sSimple[i].dwUserIdx = (DWORD)atoi((char*)pData[i].Data[0]);
			Msg.sSimple[i].dwCharNo = (DWORD)atoi((char*)pData[i].Data[1]);
...
{
	sprintf(txt, "EXEC TP_CharacterCreateInfoFromMN %d, %d", dwMemberNum, dwStartIdx );	
...
void RCharacterCreateInfoFromMN( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	{
		Msg.sInfo[i].dwCharIdx = (DWORD)atoi((char*)pData[i].Data[0]);
		Msg.sInfo[i].dwUserIdx = (DWORD)atoi((char*)pData[i].Data[1]);
		strcpy( Msg.sInfo[i].sName, (char*)pData[i].Data[2] );
		Msg.sInfo[i].dwFlag = (DWORD)atoi((char*)pData[i].Data[3]);
		...
		startIdx	= Msg.sInfo[i].dwCharIdx;
		userIdx		= Msg.sInfo[i].dwUserIdx;
	}

	if ( count >= 100 )
	{
		Msg.bEnd = FALSE;
		CharacterCreateInfoFromMN( connectIdx, tempIdx, userIdx, startIdx );
	}
	else
	{
		Msg.bEnd = TRUE;
...
void RCharacterInvenInfo2(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	DWORD dwCharIdx = (DWORD)atoi((char*)pData[0].Data[eCI_ObjectID]);
	DWORD dwStartIdx = 0;
	for( DWORD i = 0; i < count; i++ )
	{
		dwStartIdx = (DWORD)atoi((char*)pData[i].Data[eCI_DBIDX]);
	
		ITEMBASE Item;
		Item.dwDBIdx =(DWORD) atoi((char*)pData[i].Data[eCI_DBIDX]);
		Item.wIconIdx = atoi((char*)pData[i].Data[eCI_IDX]);
		Item.Position = atoi((char*)pData[i].Data[eCI_Position]);
		Item.Durability = (DWORD)atoi((char*)pData[i].Data[eCI_Durability]);
		Item.QuickPosition = atoi((char*)pData[i].Data[eCI_QPosition]);
		Item.ItemParam = (DWORD)atoi((char*)pData[i].Data[eCI_Param]);
		Item.RareIdx = (DWORD)atoi((char*)pData[i].Data[eCI_Param+1]);
...
void RCharacterShopWareItemInfo( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < dwCount; ++i )
	{
		...
		Item.dwDBIdx = (DWORD)atoi((char*)pData[i].Data[0]);
		Item.wIconIdx = atoi((char*)pData[i].Data[1]);
		Item.Position = atoi((char*)pData[i].Data[2]);
		Item.Durability = (DWORD)atoi((char*)pData[i].Data[3]);
		Item.ItemParam = (DWORD)atoi((char*)pData[i].Data[4]);
...
void RSearchGuild( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	{
		GUILDBASEINFO Info;
		Info.dwGuildIdx = (DWORD)atoi((char*)pData[i].Data[0]);
		strcpy( Info.sGuildName, (char*)pData[i].Data[1] );
		Info.dwMasterIdx = (DWORD)atoi((char*)pData[i].Data[2]);
		strcpy( Info.sMasterName, (char*)pData[i].Data[3] );
		Info.dwGuildLevel = (DWORD)atoi((char*)pData[i].Data[4]);
		Info.dwGuildMap = (DWORD)atoi((char*)pData[i].Data[5]);
		Info.dwUnionIdx = (DWORD)atoi((char*)pData[i].Data[6]);
		Info.dwGuildMoney = (DWORD)atoi((char*)pData[i].Data[7]);
...
void RSearchGuildInfo( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	else
	{
		...
		GUILDBASEINFO Info;
		Info.dwGuildIdx = (DWORD)atoi((char*)pData[0].Data[0]);
		strcpy( Info.sGuildName, (char*)pData[0].Data[1] );
		Info.dwMasterIdx = (DWORD)atoi((char*)pData[0].Data[2]);
		strcpy( Info.sMasterName, (char*)pData[0].Data[3] );
		Info.dwGuildLevel = (DWORD)atoi((char*)pData[0].Data[4]);
		Info.dwGuildMap = (DWORD)atoi((char*)pData[0].Data[5]);
		Info.dwUnionIdx = (DWORD)atoi((char*)pData[0].Data[6]);
		Info.dwGuildMoney = (DWORD)atoi((char*)pData[0].Data[7]);
...
void RSearchGuildMemberInfo( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < count; ++i )
	{
		if( atoi((char*)pData[i].Data[3]) < 5 )
		{
			...
			Info.dwChrIdx = (DWORD)atoi((char*)pData[i].Data[0]);
			strcpy( Info.sName, (char*)pData[i].Data[1] );
			Info.dwLevel = (DWORD)atoi((char*)pData[i].Data[2]);
			dwGuildIdx = (DWORD)atoi((char*)pData[i].Data[4]);
			Info.dwPosition = (DWORD)atoi((char*)pData[i].Data[5]);
...
void RSearchGuildWareItemInfo( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	{
		ITEMBASE Item;
		Item.ItemParam = (DWORD)atoi((char*)pData[i].Data[0]);		// guildidx
		Item.dwDBIdx = (DWORD)atoi((char*)pData[i].Data[1]);
		Item.wIconIdx = atoi((char*)pData[i].Data[2]);
		Item.Position = atoi((char*)pData[i].Data[3]);
		Item.Durability = (DWORD)atoi((char*)pData[i].Data[4]);
...
void RCharacterRareItemOptionInfo( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	DWORD UserIdx = (DWORD)atoi((char*)pData[0].Data[0]);
	DWORD CharIdx = (DWORD)atoi((char*)pData[0].Data[1]);
	DWORD StartIdx = 0;
...
void RSearchPetInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	else
	{
		...
		PETBASEINFO Info;
		Info.dwPetDBIdx = (DWORD)atoi((char*)pData[0].Data[0]);			//pet index
		Info.dwMasterIdx = (DWORD)atoi((char*)pData[0].Data[1]);			//user id index
		Info.dwSummonItemDBIdx = (DWORD)atoi((char*)pData[0].Data[2]);		//아이템 고유번호
		Info.dwKind = (DWORD)atoi((char*)pData[0].Data[3]);				//pet 종류
		Info.dwGrade = (DWORD)atoi((char*)pData[0].Data[4]);				//pet 단계
		Info.dwStamina = (DWORD)atoi((char*)pData[0].Data[5]);				//pet stamina
		Info.dwFriendShip = (DWORD)atoi((char*)pData[0].Data[6]);			//pet 친밀도
		...
		Info.dwCharIdx = (DWORD)atoi((char*)pData[0].Data[11]);			//pet을 소유하고 있는 캐릭터 index
...
void RSearchPetList( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	else
	{
		...
		for( DWORD i = 0; i < count; ++i )
		{
			Msg.sPetList[i].dwPetDBIdx = (DWORD)atoi((char*)pData[i].Data[0]);		// pet index
			Msg.sPetList[i].dwKind = (DWORD)atoi((char*)pData[i].Data[1]);			// pet 종류
			Msg.sPetList[i].dwGrade = (DWORD)atoi((char*)pData[i].Data[2]);		// pet 등급
			Msg.sPetList[i].dwStamina = (DWORD)atoi((char*)pData[i].Data[3]);		// pet stamina
			Msg.sPetList[i].dwFriendShip = (DWORD)atoi((char*)pData[i].Data[4]);	// pet 친밀도

			Msg.sPetList[i].dwChrIdx = (DWORD)atoi((char*)pData[i].Data[5]);		// pet을 소유한 캐릭터 index
...
void RSearchPetWareItemInfo( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	dwPetidx = (DWORD)atoi((char*)pData[0].Data[0]);
...
void RSearchDelCharList( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	else
	{
		...
		for( DWORD i = 0; i < count; ++i )
		{
			...
			Msg.sDelcharList[i].wflag = atoi((char*)pData[i].Data[3]);

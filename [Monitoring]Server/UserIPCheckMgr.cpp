...
BOOL CUserIPCheckMgr::CheckPower( DWORD dwConnectionIndex, WORD wPower )
{
	if( dwConnectionIndex > 99 )
		return FALSE;

	if( m_IDArray[dwConnectionIndex].wPower != wPower )
	{
//		MSGROOT msg;
//		msg.Category = MP_RMTOOL_CONNECT;
//		msg.Protocol = MP_CHECKPOWER_ERROR;
...
WORD CUserIPCheckMgr::GetPower( DWORD dwConnectionIndex )
{
	if( dwConnectionIndex > 99 )
		return 999;

	return m_IDArray[dwConnectionIndex].wPower;
}

void CUserIPCheckMgr::RegistID( DWORD dwConnectionIdx, DWORD dwIdx, char* pName, WORD wPower )
{
	...
	m_IDArray[dwConnectionIdx].wPower = wPower;
}

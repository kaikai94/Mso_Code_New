...
BOOL CBattle_GTournament::Judge()
{
	...
	if( m_GTInfo.m_dwFightTime == 0 )
	{
		...
		{
			if( JudgeMemberKill() == FALSE )		
			if( JudgeMemberLevel() == FALSE )	
			if( JudgeMemberLevelExp() == FALSE )
			{
				// Random
				DWORD data = rand()%2;
				Victory( data, 1-data );
...
void CBattle_GTournament::Victory(int WinnerTeamNum,int LoserTeamNum)
{
	...
	m_vTitleScale.x = 1.0f;
	m_vTitleScale.y = 1.0f;
	m_vTitlePos.x = (1024 - 512 * m_vTitleScale.x ) / 2;	
	m_vTitlePos.y = 100;
	m_RenderTime = 10000;
...
void CBattle_GTournament::CreateWaitPlace()
{
	...
	//[江湖争霸开始时在此报错，疑似效果文件问题][By:十里坡剑神][QQ:112582793][2018/3/6]

...
void CBootManager::AddSelfBootList(WORD MyServerKind, WORD MyServerNum,  CServerTable * pServerTable)
{
	...
	if(fp == NULL)
	{
		MessageBox(NULL,"serverset.txt文件不存在,服务即将退出",0,0);
		exit(0);
		return;
	}
	int serversetnum;
...
BOOL CBootManager::StartServer(CNetwork * pNet, 	SERVERINFO * pSelfServerInfo)
{
	if(pSelfServerInfo->wServerKind!=RECONNECT_SERVER)
	{
		...
		{
			g_Console.LOG(4, "[错误]服务端服务进程启动失败:IP=%s	端口=%d", pSelfServerInfo->szIPForServer, pSelfServerInfo->wPortForServer);
			return FALSE;
		}
		else
		{
			g_Console.LOG(4, "服务端服务进程启动完成:IP=%s	端口=%d", pSelfServerInfo->szIPForServer, pSelfServerInfo->wPortForServer);
		}
	}
		...
		||pSelfServerInfo->wServerKind == RECONNECT_SERVER)
	{
		...
		{
			g_Console.LOG(4, "[错误]客户端服务进程启动失败:IP=%s	端口=%d", pSelfServerInfo->szIPForUser, pSelfServerInfo->wPortForUser);
			return FALSE;
		}
		else
		{
			g_Console.LOG(4, "客户端服务进程启动完成:IP=%s	端口=%d", pSelfServerInfo->szIPForUser, pSelfServerInfo->wPortForUser);
		}
	}
	return TRUE;
}

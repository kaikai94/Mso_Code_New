...
eTCONSOLE_ERROR __stdcall CTConsole::CreateConsole(MHTCONSOLE_DESC * pDesc, HWND * hWndOut)
{
	...
	if(pDesc->szConsoleName)
		strncpy_s(m_DESC.szConsoleName, sizeof(m_DESC.szConsoleName), pDesc->szConsoleName, CONSOLENAME_SIZE);
	m_DESC.dwRefreshRate				= pDesc->dwRefreshRate;
	m_DESC.wLogFileType					= pDesc->wLogFileType;
	if(pDesc->szLogFileName)
		strncpy_s(m_DESC.szLogFileName, sizeof(m_DESC.szLogFileName), pDesc->szLogFileName, LOG_FILENAME_SIZE);
	...
	for(int i = 0 ; i < m_DESC.nCustomMunuNum ; ++i)
	{
		if(pDesc->pCustomMenu[i].szMenuName)
			strncpy_s(m_DESC.pCustomMenu[i].szMenuName, sizeof(m_DESC.pCustomMenu[i].szMenuName), pDesc->pCustomMenu[i].szMenuName, MUNU_CUSTOMNAME_SIZE);
...
void __stdcall CTConsole::OutputFile(char * szString, int strLen)
{
	...
	strncpy_s(&m_FileBuffer[m_dwFileBufferCP], sizeof(m_FileBuffer[m_dwFileBufferCP]), szString, strLen);

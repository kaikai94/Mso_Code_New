...
//041213 KES decreaserate
	...
	while(pList = pTargetList->GetNextTargetList())
	{
		...
		if(pOperator->GetBattle()->IsEnemy(pOperator,pList->pObject))
		{
			...
			float fDecreaseDamageRate = 1.0f;	
			if( bDecrease )
			{
				VECTOR3 TargetPos;
			...
			else
			{
				...
				ATTACKMGR->Attack(m_bMugong,
					...
					m_CriticalRate,
					&DamageInfo,
					TRUE, fDecreaseDamageRate, AmplifiedPowerAttrib );	
			}
												   ...
												   MSG_SKILLOBJECT_ADD* pAddMsg,float SkillTreeAmp)
{
	...
	while(pList = pTargetList->GetNextTargetList())
	{
		...
		{
			int nSeed = rand()%100;
			if(pList->pObject->GetObjectKind()>=eObjectKind_Monster&&pList->pObject->GetObjectKind()<=eObjectKind_FieldSubMonster)
			{//[混血技能怪物可用控制修改][By:十里坡剑神][QQ:112582793][2019/3/10--16:]
				if(!GAMERESRCMNGR->IsCanUseAAKLife(((CMonster*)(pList->pObject))->GetMonsterKind()))

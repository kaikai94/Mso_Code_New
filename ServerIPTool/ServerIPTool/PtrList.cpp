...
//				CPlex	class									//
//////////////////////////////////////////////////////////////////
CPlex1* PASCAL CPlex1::Create(CPlex1*& pHead, UINT nMax, UINT cbElement)
{
	CPlex1* p = (CPlex1*) new BYTE[sizeof(CPlex1) + nMax * cbElement];
	p->pNext = pHead;
	pHead = p;
	return p;
}

void CPlex1::FreeDataChain()
{
	CPlex1* p = this;
	while (p != NULL)
	{
		BYTE* bytes = (BYTE*) p;
		CPlex1* pNext = p->pNext;
		delete[] bytes;

...

{
	...
	while( pANRoom = m_htAutoNoteRoom.GetData() )
	{
		if( pANRoom->IsTimeOver() )
		{
			...
			if( pANRoom->GetAutoNoteRoomState() == eANRS_WAITANSWER )	
			{
				if( pAutoPlayer )
				{
	...
	case MP_AUTONOTE_ASKTOAUTO_SYN:
		{
			...
			{
				error = eAutoNoteError_AlreadyAsking;			
				goto AutoNoteAskError;
			}
			...
			if( pAutoPlayer == NULL )
			{
				error = eAutoNoteError_CantFind;				
				goto AutoNoteAskError;
			}

			if( pAutoPlayer->GetAutoAskPlayerIdx() )		
			{
				error = eAutoNoteError_AlreadyAsked;			
				goto AutoNoteAskError;
			}

			if( gCurTime > pAutoPlayer->GetLastAttackTimeForAutoNote() + 10*1000 )
			{
				error = eAutoNoteError_NotProperState;			
				goto AutoNoteAskError;
			}

...
{
	if (m_RankMemberIdx[0] != PlayerIDX&&m_RankMemberIdx[1] != PlayerIDX)
		return FALSE;
	else
		return TRUE;
}
	...
	while(pos)
	{
		...
		if( pInfo )
		{
			...
			if(pPlayer)
			{
				...
				pPlayer->SetNickName("");
				pPlayer->ClearKillTimes();	//[°ï»á½âÉ¢ºóÇå¿Õ³ÉÔ±»÷É±¼ÆÊý][BY:Ê®ÀïÆÂ½£Éñ][QQ:112582793][2019-9-4][17:18:07]
...
void CGuild::SendUnionNote(CPlayer *pPlayer, char* note)
{
	...
	if (m_RankMemberIdx[1])
	{
		GUILDMEMBERINFO* pInfo = GetMemberInfo(m_RankMemberIdx[1]);
		SendNote(pPlayer, pInfo->MemberName, note);
	}
}

	...
	case eObjectState_Die:
		{
			...
			{
				char buf[256];
				sprintf(buf, "StartObjectState1(NowState : %d, ChangeState : %d)", pObject->GetState(), State);
				ASSERTMSG(0, buf);
			}
		}
		return FALSE;
	...
	case eObjectState_Tactic:
		{
			...
			{
				char buf[256];
				sprintf(buf, "StartObjectState2(NowState : %d, ChangeState : %d)", pObject->GetState(), State);
				ASSERTMSG(0, buf);

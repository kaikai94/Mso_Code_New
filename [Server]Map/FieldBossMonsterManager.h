...
//[BOSS掉落物品信息结构][By:十里坡剑神][QQ:112582793][2018/1/10]
typedef struct stFieldBossDropItem
{

	char    Name[MAX_FIELDBOSS_DROPITEM_NUM][MAX_ITEMNAME_LENGTH+1];
...
public:
	...
	void AddRegenPosition(float x, float z);
	VECTOR3 GetRegenPosition(int index = -1);	
	WORD GetRegenPositionCount() { return m_RegnePosCount; }
	
	void BossDead(CFieldBossMonster* pBoss);
	void SubDead(CFieldSubMonster* pSub);	

	void AddFieldBossMonster( CFieldBossMonster* pBoss );

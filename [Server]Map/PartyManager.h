...
public:
	...
	void MasterToPartyRequest(CPlayer* pPlayer, MSG_NAME_DWORD2* pMsg);	
	void MasterToPartyRequestErr(DWORD dwPlayerID, DWORD dwErrIndex);	
	void NotifyPartyInfoSyn(DWORD dwPartyID);		
	void NotifyPartyInfo(PARTY_INFO* pPartyInfo);	
	void PartyMatchingInfo(CPlayer* pPlayer, MSG_WORD* pMsg);	
	void Process();	
#ifdef _XDYY_
	void PartVoiceNotify(MSG_DWORD3* pMsg,int MsgSize);
#endif
};

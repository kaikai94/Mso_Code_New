...
char* AddComma(MONEYTYPE dwMoney)
{
	static char buf[128];
	sprintf( buf, "%lld", dwMoney );
	AddComma( buf );
	return buf;
}
...
// magi82(37) 맵 성 데이터 - 스크립트로 빠지면서 해당 함수는 주석 처리(더이상 안쓰임)
/*
...
{
#ifdef _MHCLIENT_
	//ASSERT( ITEMMGR->GetItemInfo(iconIdx) );	
...
char* GetCurTimeToString()
{
	...
	SYSTEMTIME ti;
	GetLocalTime(&ti);
	sprintf(temp,"%d墩/%d휑 %d珂%d롸%d.%d취",		// 墩/휑珂：롸：취
		ti.wMonth,ti.wDay,
